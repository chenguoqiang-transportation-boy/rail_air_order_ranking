...
class TwoStageOptimizer:
    ...
    - 决策时在“最优联运(期望成本)”与“DUMMY成本”之间选择更低者
    """

    DUMMY_ID = "__DUMMY__"
    TRANSFER_TIME = 75.0

    def __init__(self, cost_calculator: CostCalculator, score_weights: Optional[Dict[str, float]] = None):
        self.cost_calc = cost_calculator
        self.horizon_time: float = None

        # 权重：用于 priority_score
        # 你可以从随机搜索脚本注入不同权重
        self.w = score_weights or {
            "regret": 1.0,
            "window": 1.0,
            "scarcity": 1.0,
            "tightness": 1.0,
    ...
    def solve(self, input_data: Dict, current_time: float) -> Dict:
        ...
        # 如果你希望 regret 的“下一周期”严格用 period_len，可从 input_data 传入
        # self.period_len = input_data.get("period_len", 30.0)
    ...
    def _priority_score(self, order: Order, current_time: float, trains: Dict, flights: Dict, indexes: Dict) -> float:
        ...
        period = getattr(self, "period_len", 30.0)
