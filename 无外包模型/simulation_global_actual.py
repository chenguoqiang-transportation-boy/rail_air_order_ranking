...
from optimization_global_actual import GlobalOptimizer
...
class Simulator:
    ...
    def run(self):
        ...
        # ==========================================
        # 阶段一：全局确定性规划 (Global Deterministic Optimization)
        # ==========================================
        print(">>> 正在构建全局优化模型...")

        # 这里的关键是：self.state.trains 和 flights 在 _init_network 时
        # 已经被注入了 actual_arrival / actual_departure (上帝视角延误)
        # 所以直接传进去即可

        opt_input = {
            'all_orders': self.all_orders,  # 传入所有订单
            ...
            # DELETE: 'scenarios': ... (不需要了)
        }

        # 替换为新的 GlobalOptimizer
        self.optimizer = GlobalOptimizer(self.cost_calc)
        ...
        # ==========================================
        # 阶段二：结果回放 (Replay / Calculation)
        # ==========================================
        # 全局优化已经给出了最优解，剩下的只是为了生成统计报表和日志
        # 我们按照时间轴“播放”一遍，把 assignments 填进去
        ...
        while self.state.current_time < self.max_time:
            ...
            # 2. 获取新订单 (仅用于回放逻辑，实际上决策早就做好了)
            batch = self._get_new_orders(t, self.period_len)
            self.state.add_new_orders(batch)

            # 3. 应用全局决策
            for oid in self.state.pool_new:
                order = self.state.get_order(oid)

                # 直接查表
                if oid in assignment_lookup:
                    decision = assignment_lookup[oid]
                    mode = decision['mode']
                    ...
                    if mode == 'INTERMODAL':
                        ...
                        # 资源扣减
                        # 注意：虽然全局优化约束了容量，但在回放时最好也更新一下状态以防万一
                        ...
                        for s in train.stations:
                            if s.origin_id == order.origin:
                                order.actual_pickup_time = s.actual_arrival  # 近似
                                break

                    else:  # AD_HOC
                        order.mode = TransportMode.AD_HOC
                        order.status = OrderStatus.AD_HOC_MODE
                        order.actual_delivery_time = order.deadline
                else:
                    # 如果漏掉了（理论上不应该），默认外包
                    order.mode = TransportMode.AD_HOC
                    order.status = OrderStatus.AD_HOC_MODE
