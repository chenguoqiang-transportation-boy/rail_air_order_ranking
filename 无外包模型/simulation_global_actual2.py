...
from data_generation import (
    DataLoader, NetworkInitializer, OrderGenerator,
    DelayModel, Costs, Order, TransportMode, OrderStatus
)
from cost_calculator import CostCalculator
from optimization_global_actual2 import GlobalOptimizer
...
class SimulationState:
    ...
    def __init__(self):
        ...
        # 订单集合 (使用 ID)
        self.pool_new: List[str] = []
        self.pool_waiting_for_train: set = set()
        self.pool_on_train: set = set()
        self.pool_transferring: set = set()
        self.pool_waiting_for_flight: set = set()
        self.pool_on_flight: set = set()

        # 无联运线路（DUMMY）订单池：不参与物理运输，只到 max_time 才结算
        self.pool_unserved: set = set()

        self.pool_finished: List[str] = []
...
class TransportationEngine:
    ...
    def process_events(self):
        ...
        # 3) TRANSFERRING -> WAITING_FOR_FLIGHT（不再外包分支）
        ...
        # 4) WAITING_FOR_FLIGHT -> ON_FLIGHT（补 actual_flight_time）
...
class Simulator:
    ...
    def run(self):
        print(f"开始全局优化仿真: {len(self.all_orders)} 订单")
        self.history = []
        ...
        opt_input = {
            ...
            "available_trains": self.state.trains,
            "available_flights": self.state.flights,
            "indexes": {
                "train_origin": self.state.train_origin_idx,
                "flight_dest": self.state.flight_dest_idx
            },
            # 你指定：用 max_time 作为时间结束
            "horizon_time": self.max_time
        }

        self.optimizer = GlobalOptimizer(self.cost_calc)
        global_plan = self.optimizer.solve(opt_input)
        assignments = global_plan["order_assignments"]

        print(f">>> 全局优化完成。统计: {global_plan['statistics']}")
        ...
        while self.state.current_time < self.max_time:
            ...
            # 3) 应用全局决策（只联运；train/flight=None => DUMMY）
            for oid in list(self.state.pool_new):
                order = self.state.get_order(oid)
                decision = assignment_lookup.get(oid)

                if decision is None:
                    # 理论上不应发生：优化器已保证每订单至少一条 route（含 DUMMY）
                    # 兜底：当 DUMMY
                    order.mode = TransportMode.INTERMODAL
                    order.assigned_train = None
                    order.assigned_flight = None
                ...
                order.assigned_flight = decision["flight"]

                if order.assigned_train is None and order.assigned_flight is None:
                    # DUMMY：进入 unserved 池，等 max_time 结算
                    order.status = OrderStatus.WAITING
                    order.actual_delivery_time = self.max_time
                    self.state.pool_unserved.add(oid)
                else:
                    ...
                    for s in train.stations:
                        if s.origin_id == order.origin:
                            order.actual_pickup_time = s.actual_arrival
                            break
        ...
        # ====== 关键补丁：补一个最终时刻 tick，确保 unserved 在 max_time 被结算 ======
    ...
    def _update_pools_after_decision(self):
        for oid in list(self.state.pool_new):
            ...
            if order.mode == TransportMode.INTERMODAL:
                # DUMMY：不进等待上车池（已在 pool_unserved）
                if order.assigned_train is None and order.assigned_flight is None:
